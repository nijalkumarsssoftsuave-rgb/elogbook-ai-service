...
def test_any_single_filter_makes_the_set_non_empty(kwargs: dict) -> None:
    ...
    """
    assert not QueryFilters(**kwargs).is_empty
...
def test_a_single_day_range_is_valid() -> None:
    ...
    than for nothing.
    """
    filters = QueryFilters(date_from=date(2026, 7, 1), date_to=date(2026, 7, 1))

    assert filters.date_from == filters.date_to


def test_an_open_ended_range_is_valid_in_either_direction() -> None:
    assert QueryFilters(date_from=date(2026, 7, 1)).date_to is None
    assert QueryFilters(date_to=date(2026, 7, 31)).date_from is None
...
def test_a_backwards_date_range_is_rejected() -> None:
    with pytest.raises(ValidationError, match="date_from must not be after date_to"):
        QueryFilters(date_from=date(2026, 7, 31), date_to=date(2026, 7, 1))
...
def test_a_blank_filter_value_is_rejected(blank: str) -> None:
    ...
    with pytest.raises(ValidationError, match="must not be blank"):
        QueryFilters(area_ids=["north", blank])


def test_a_malformed_date_is_rejected() -> None:
    with pytest.raises(ValidationError):
        QueryFilters.model_validate({"date_from": "the first of July"})
...
def test_an_unknown_filter_key_is_rejected_rather_than_ignored() -> None:
    ...
    """
    with pytest.raises(ValidationError):
        QueryFilters.model_validate({"area_id": "north"})


def test_a_filter_value_that_is_not_a_list_is_rejected() -> None:
    with pytest.raises(ValidationError):
        QueryFilters.model_validate({"area_ids": "north"})
...
def test_an_unbounded_filter_list_is_rejected() -> None:
    ...
    """
    with pytest.raises(ValidationError):
        QueryFilters(area_ids=[f"area-{n}" for n in range(51)])
