...
#
# Every entry declares an area, department, company, date, status and tag list, because
# retrieval matches all of those fail-closed: a document that does not declare an attribute
# cannot satisfy a filter on it, so an untagged corpus would make every restricted role and
# every filtered request resolve to nothing at all. The attributes are metadata only -- no
# document's *text* changes -- which is what keeps the BM25 index, and therefore every
# measured ranking, exactly as it was.
